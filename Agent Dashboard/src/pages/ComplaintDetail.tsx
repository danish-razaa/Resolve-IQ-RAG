...
import { Complaint, getStoredComplaints } from "@/data/mockData";
import { ArrowLeft, Mail, Phone, Smartphone, Building2, Globe, Sparkles, Loader2 } from "lucide-react";
import { useState } from "react";
import { useToast } from "@/hooks/use-toast";
import { processComplaintApi } from "@/lib/api";
...
const getInitialStatus = (complaint: Complaint): Status => {
...
const ComplaintDetail = () => {
  ...
  const handleRegenerate = async () => {
    ...
    try {
      const res = await processComplaintApi(complaint.customerName, complaint.unmaskedText);
      if (res.agent_response) {
        setReply(res.agent_response);
        toast({
          title: "AI Response Regenerated",
          description: `Fresh resolution generated by ${res.domain || "AI"} Agent`,
        });
      }
    } catch (err: any) {
      toast({
        title: "Live API Unavailable",
        description: err.message || "Failed to reach live backend. Using existing draft.",
        variant: "destructive",
      });
    } finally {
          ...
          <div className="bg-card border border-border rounded-xl p-5 space-y-4">
            ...
            <div className="space-y-3 text-sm">
              ...
              <Row label="Severity" value={
                <span className={`px-2 py-0.5 rounded text-xs font-semibold ${severityColor[complaint.severity] || "bg-secondary text-foreground"}`}>
